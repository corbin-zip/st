...
- corner-dragging to tiny sizes no longer chains stale prompt fragments into one growing junk line: a row rewritten from column 0 after a carriage return severs the previous row's wrap link, and a new `resizedebounce` (config.h/Xresources, default 80 ms) tells the shell about a new size only once the geometry settles, so a drag triggers one prompt redraw instead of dozens (stranded single prompt lines during extreme resize storms remain possible — the artifact all reflowing terminals share without OSC 133 shell integration)
